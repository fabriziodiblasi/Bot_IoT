...
def chiudi_valvola():
    ...
    mex_err, valv_err = controllo_errore_chiusura_apertura()
    return mex_err, valv_err
...
def start_control(message):
    ...
    if ignora == 0:
        if chiuso == 0:
            # richiamerò la funzione che manda il messaggio di chiusura ad arduino
            chiuso = 1
            markdown = """
                       *Valvola chiusa*
                       """
            bot.send_message(message.chat.id, markdown, parse_mode="Markdown")
            mex, flag_valvola = chiudi_valvola()
            if flag_valvola == 1:
                 bot.send_message(message.chat.id, mex)
        else:
            markdown = """
...
def start_control(message):
    ...
    if chiuso == 1:
        ...
        mess, flag_valvola = apri_valvola()
        if mess != "":
            #chiuso = 0
...
def start_control(message):
    ...
    if chiuso == 0:
        ...
        mess, flag_valvola = chiudi_valvola()
